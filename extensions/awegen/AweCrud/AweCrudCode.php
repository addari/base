...
/*
    ...
    $str .= "<?php echo CHtml::checkBoxList('{$modelClass}[{$relatedModelClass}]',\n
        array_map('Awecms::getPrimaryKey',\$model->{$relatedModelClass}),\n
        CHtml::listData({$relation[1]}::model()->findAll(),'{$foreign_pk}',\n
            '{$foreign_identificationColumn}'),
array( 'attributeitem' => 'id',\n
 'separator' => '&nbsp;&nbsp;&nbsp;',\n
 'container' => 'div',\n
...
return "echo \$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50,'class'=>'span5'))";
...
return $string;
    ...
    $variable_lenguaje = '<?php echo Yii::app()->language ?>';
    ...
    if ($column->type !== 'string' || $column->size === null)
        ...
        if (($size = $maxLength = $column->size) > 60)
            $size = 60;
        return "echo \$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength,'class'=>'span5'))";
    }
}
}
}
